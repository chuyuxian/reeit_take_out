...
import com_reggie.utils.ValidateCodeUtils;
...
public class UserController {
    ...
    private R<String> sendMsg(@RequestBody User user, HttpSession session){
        ...
        //isNotEmpty：不为空,才继续执行里面的方法，为空直接返回提示失败
        if(StringUtils.isNotEmpty(phone)){
            //生成随机的4位验证码，可以设置生成几位验证码
            String code = ValidateCodeUtils.generateValidateCode(4).toString();
            log.info("code={}",code);

            //调用阿里云提供的短信服务API完成发送短信，没有注册个人的，所以注释起来了
    ...
    private R<User> login(@RequestBody Map map, HttpSession session){
        ...
        //获取验证码，这个是前端输入的，不是我们自己生成的
        String code = map.get("code").toString();

        //从Session中获取保存的验证码，自己生成的
        ...
        if(codeInSession != null && codeInSession.equals(code)){
            ...
            //查询表中是否有这个用户，如果不需要注册，如果没有自动注册，这个方法可以进去看看
